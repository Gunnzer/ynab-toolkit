...
describe("shared expenses", () => {
  ...
  test("an already-split transaction is always reported as skipped, never split", () => {
    ...
    // as the genuinely shared portion. Splitting just that leg was tried
    // and removed (a real production category-display bug traced back to
    // the delete+recreate it required) - already-split transactions are
    // always skipped now, with no way to opt into converting them.
    const dinner = {
    ...
    const result = shared.scan([dinner], [rule], "2025-01-01", "2030-12-31", 0.5);
    assert.equal(result.planned.length, 0);
    assert.equal(result.skippedAlreadySplit, 1);
    assert.deepEqual(
      result.skipped.map((s) => [s.transaction.id, s.reason]), [["d1", "already split"]]);
  });
});
