...
**Gotcha, real bug reported live - the Undo dialog defaulted to "restore everything":** `undo()`'s checkbox list (`shared.js`) used to set `box.checked = true` for every backed-up split, contradicting its own hint text ("tick the ones to restore"). Opening it meaning to undo one specific transaction silently left every *other* pending backup ticked too, so "Restore selected" reverted all of them - reported live as "Ready to Assign is messed up and it undid more than it should." Restoring a batch of transactions (especially backdated ones) back to their original category is exactly what shifts the current month's RTA, per the RTA Tracker note above. Fixed: rows start unticked; a "select all" checkbox in the check column's own header (same shortcut pattern as Reports' `chooseCategories()` group checkbox) still restores the one-click "undo everything" option, but only as something you deliberately click. The dialog's list is also now newest-first and grouped under a `tr.group-row` date heading (`sorted`, a display-order copy - `stored`'s own order still feeds `saveBackups()`/`undoFromBackup()`, neither of which cares about order, only ids) rather than whatever order backups happened to accumulate in, since a growing list was hard to scan for the one or two actually meant to be undone.
...
**Gotcha, real bug reported live:** stripping emoji with only `\p{Extended_Pictographic}` left a stray box/circle glyph behind for a lot of real-world emoji, reported live as a leading mark with nothing visible attached to it in front of a plain category name. Many emoji are actually a plain text character (a dumbbell, a heart) plus an invisible variation selector (U+FE0F) riding next to it that tells a renderer "show this as emoji, not text" - `\p{Extended_Pictographic}` only matches the base character, so the selector survived the strip with nothing left to modify. `EMOJI` in `spending_export.js` strips the variation selectors (U+FE0E/U+FE0F), the zero-width joiner that stitches a multi-part emoji together (U+200D), and the skin-tone modifiers (U+1F3FB-U+1F3FF) alongside `\p{Extended_Pictographic}` itself, in one regex.
...
- **Gotcha:** `.table-wrap`'s `overflow-x: auto` (for a wide table's
  ...
  *page's* scroll, because the CSS spec forces `overflow-y` to compute as
  ...
  need it.
- **Gotcha, reported live:** `.scroll-table`'s vertical scrollbar spans the
  whole scrolling box - header included, since the sticky `th` lives inside
  the same box - so at `scrollTop: 0` the thumb sits right alongside the
  header row instead of only alongside the data beneath it, reading as the
  scrollbar overlapping the header bar. Chromium/WebKit let a
  scrollbar-*track* pseudo-element carry its own margin, so
  `.scroll-table::-webkit-scrollbar-track { margin-top: 40px; }` (`app.css`,
  40px matching `th`'s own height everywhere) reserves the header's height
  out of the track, keeping the thumb confined to the row area. No
  equivalent exists in Firefox - there it just falls back to the
  unmodified full-height scrollbar, same as before this fix, never worse.
