...
**Save file format:** `toOfx(rows, { accountName })` writes the converted rows out as a QFX/OFX 1.02 SGML file, an alternative to the existing `toCsv(rows)` - `bank.js` has a small format dropdown (`SAVE_FORMATS`, persisted as `settings.saveFormat`, labels are bare "CSV"/"QFX" - no parenthetical, at explicit user request) next to the save button, picking the serializer, file extension and MIME type together. (A CSV/TSV delimiter choice was tried here too, then removed at explicit user request - only QFX was actually wanted alongside CSV.) **Gotcha:** the save button used to be labelled "Save as...", the same label the pre-existing bank-preset save button (`presetSaveButton`, in the "Bank" card above) already uses - two buttons with identical text broke a `find(b => b.textContent === "Save as...")` style lookup (caught during live verification, matched the wrong button and silently opened the preset-naming dialog instead of downloading). Renamed to "Save file..." to disambiguate; keep that in mind before reusing "Save as..." as a label anywhere else on this page.
