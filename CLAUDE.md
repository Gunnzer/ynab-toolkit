...
- **Gotcha (bigger one, confirmed live):** `position: sticky` on a `<th>`
  ...
  table's bounded/scrolling panel (`.preview-table`, see below), this showed
...
- **Gotcha:** a column filter's `.filter-popup` used to be appended as a
  ...
  copied from). Inside `.preview-table`'s bounded/scrolling panel that
  ...
  Any future popup opened from inside `.preview-table` (or any other
...
- **Gotcha:** `.table-wrap`'s `overflow-x: auto` (for a wide table's
  ...
  *page's* scroll, because the CSS spec forces `overflow-y` to compute as
  ...
  nothing. Don't fight this - embrace it: Bill Splitting's Preview table
  (`.preview-table` in `app.css`, added via `preview.classList.add(...)` in
  `splitsheet.js`) gives the wrapper a real `max-height` + `overflow-y:
  auto`, turning it into a proper scrollable panel with a frozen header
  instead of growing the whole page. Any other table that wants a sticky
  header while also needing horizontal scroll should follow the same
  pattern (its own bounded-height class), not try to make `overflow-y`
  stay `visible` on `.table-wrap`.
